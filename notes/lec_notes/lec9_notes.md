...
σ：
>  控制随机变量波动大小的参数（离它自己的平均值（期望）通常会偏多远）。

σ 越大，说明 reward 可能波动得越厉害，所以估计 Q(a) 就越不确定

它是一个能够控制这个随机变量尾部波动的上界参数，也就是随机变量 X **跑到离平均值很远的地方的概率有多大**

如果 σ 小，比如： σ=1

那么：
$P(|X-\mu|\ge\eta)\le2e^{-\eta^2/2}$
